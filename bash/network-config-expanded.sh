...
# create array to hold interface information
my_interfaces=()

# count how many lines in the interfaces output but ignore the first line because it's the headings
num_interfaces=$(( $(nmcli device status | wc -l ) -1 ))

#-------------------------
# Gather interface names
#------------------------
# loop through each line of interface info and take out the interface name but ignore loopback
# place in an array

for (( x=0; x < num_interfaces; x++)); do

  a=$(nmcli device status | sed -n "$(($x+2))"'p' | awk '{print $1}')

  if [ $a != "lo" ]; then
    my_interfaces[$x]="$a"
  fi

  #echo at array $x = "${my_interfaces[$x]}" and "${my_interfaces[@]}"
done
...
================================
Number of interfaces(not including loopback): $(($num_interfaces-1))
Current interfaces: ${my_interfaces[@]}
Array length: ${#my_interfaces[@]}
EOF
...
for ((x=0; x <n_int; x++)); do
  echo hello

interface=${my_interfaces[$x]}
