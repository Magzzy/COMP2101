...
#download tgz file of tarfile to Pictures directory if it isn't already there.
[ -f ~Pictures/pics.tgz ] || wget -q -O ~/Pictures/pics.tgz https://zonzorp.net/pics.tgz

#unpack the downloaded zipfile if it is there, then delete the local copy of the zipfile
[ -f ~/Pictures/pics.tgz ] && tar -xzf ~/Pictures/pics.tgz -C ~/Pictures && rm ~/Pictures/pics.tgz

# Make a report on what we have in the Pictures directory
test -d ~/Pictures && cat <<EOF
===========================================================
Found $(find ~/Pictures -type f|wc -l) files in the Pictures directory.
The Pictures directory now uses $(du -sh ~/Pictures|awk '{print $1}') space on the disk.
EOF
